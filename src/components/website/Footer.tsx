    ...
    <footer className="w-full py-8 border-t bg-neutral-50">
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col items-center mb-8">
          ...
          <nav className="flex md:hidden flex-wrap justify-center gap-2 mb-8 w-full max-w-md">
            <Link
              href="/about"
              className="flex-1 min-w-[calc(50%-0.25rem)] px-3 py-2.5 text-sm text-gray hover:text-black transition-colors text-center bg-white/50 rounded-lg border border-gray-100"
            >
              About Us
            </Link>
            <Link
            ...
            <Link
              href="/track"
              className="flex-1 min-w-full px-3 py-2.5 text-sm text-gray hover:text-black transition-colors text-center bg-white/50 rounded-lg border border-gray-100"
            >
              Track Order
            </Link>
          </nav>
          ...
          <nav className="hidden md:flex flex-wrap justify-center mb-10">
...
function SuccessMessage() {
  return (
    <div className="h-7 w-full md:w-[270px] mx-auto flex items-center justify-center gap-2 text-sm text-green">
      <CheckCircle size={16} />
...
function NewsletterForm({
  ...
  return (
    <>
      ...
      <form onSubmit={onSubmit} className={clsx("md:hidden flex flex-col gap-3 w-full", isSubmitting && "opacity-45")}>
        <input
          className="w-full h-11 px-4 rounded-lg bg-white border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-300"
          type="email"
          name="email"
        ...
        <button
          ...
          className={clsx(
            "w-full h-11 rounded-lg font-semibold text-white shadow-[inset_0px_1px_0px_0px_#ffa405] [background:linear-gradient(to_bottom,_#e29000_5%,_#cc8100_100%)] bg-[#e29000] hover:bg-[#cc8100] hover:[background:linear-gradient(to_bottom,_#cc8100_5%,_#e29000_100%)] active:shadow-[inset_0_3px_8px_rgba(0,0,0,0.14)] transition-all",
      ...
      {/* Desktop Form - Keep Original Design */}
      <form
        onSubmit={onSubmit}
        className={clsx("hidden md:block relative h-11 w-[290px]", isSubmitting && "opacity-45")}
      >
        <button
          type="submit"
          disabled={isSubmitting}
