...
import { VisibilityButton, VisibilityOverlay } from "@/components/admin/Storefront/EditCollection/VisibilityOverlay";
...
export default async function EditCollection({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const productIndexes = new Map((collection.products || []).map((product) => [product.id, product.index]));
  ...
  const { id, campaignDuration, collectionType, title, slug, bannerImages, visibility, products } =
      ...
      <div className="max-w-[768px] flex flex-col gap-10 px-5">
        <div>
          <div className="mb-6">
            ...
            <p className="text-sm md:max-w-[85%]">
              Keep track of your campaign. Upcoming, Active, or Ended. This helps you plan your marketing effectively
              and make adjustments as needed for maximum impact.
            </p>
          </div>
          <div className="w-full max-w-[400px] relative p-5 pr-2 flex items-center justify-between shadow rounded-xl bg-white">
            {Object.keys(campaignDuration || {}).length > 0 ? (
              <div className="space-y-2">
                {getStatusBadge(getCampaignStatus(campaignDuration.startDate, campaignDuration.endDate))}
            ...
            <CampaignDurationButton
              className={clsx({
                "absolute top-2 right-2": Object.keys(campaignDuration || {}).length > 0,
              })}
            />
          </div>
        </div>
        <div>
          <div className="mb-6">
            ...
            <p className="text-sm md:max-w-[85%]">
              Create a title that sticks in people's minds. Make it enticing enough to make them stop and pay attention.
              Finally, reinforce it with a short, keyword-rich slug (3-5 words).
            </p>
          </div>
          <div
            className={clsx(
        ...
        {bannerImages && bannerImages.desktopImage && bannerImages.mobileImage && (
          <div>
            <div className="mb-6">
              <h2 className="font-semibold text-xl mb-3">Banner images</h2>
              <p className="text-sm md:max-w-[85%]">
                Create a banner that demands attention. Bold imagery and a strong call-to-action can turn passive
                viewers into active customers.
              </p>
            </div>
            <div className="w-full relative p-5 pr-2 flex items-center justify-between shadow rounded-xl bg-white">
              <div className="w-[calc(100%-60px)] flex flex-col gap-8">
                <div>
                  <h3 className="text-xs text-gray mb-4">Desktop (1440x360 px)</h3>
                  <div className="w-full rounded-xl flex items-center justify-center overflow-hidden">
                    {isValidRemoteImage(bannerImages?.desktopImage) && (
                      <Image src={bannerImages?.desktopImage} alt={title} width={766} height={308} priority={true} />
                    )}
                  </div>
                </div>
                <div>
                  <h3 className="text-xs text-gray mb-4">Mobile (1080x1080 px)</h3>
                  <div className="w-full max-w-[416px] aspect-square rounded-xl flex items-center justify-center overflow-hidden">
                    {isValidRemoteImage(bannerImages?.mobileImage) && (
                      <Image src={bannerImages?.mobileImage} alt={title} width={766} height={308} priority={true} />
                    )}
                  </div>
                </div>
              </div>
        ...
        <div>
          <div className="mb-6">
            <h2 className="font-semibold text-xl mb-3">Products ({products?.length ?? 0})</h2>
            <p className="text-sm md:max-w-[85%]">
              Pick stuff that goes well together. Choose different looks, colors, and prices. Make sure there's
          ...
          >
            {products && products.length > 0 ? (
              <div className="w-[calc(100%-60px)] flex flex-wrap gap-5 justify-start">
                {products.slice(0, 3).map(({ id, slug, name, pricing, images }) => (
                  <Link
                    key={id}
